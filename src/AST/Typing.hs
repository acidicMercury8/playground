...
deduceType (Block block) tm =
  let tryTAST = deduceBlock block tm
   in case tryTAST of
        Right e -> Right e
        Left (tast, _) -> Left (TypedExpr (getLastType tast) (TBlock tast), tm)
deduceType (Def type_ name) tm =
  Left
...
deduceType (TypeCast type_ expr) tm =
  let tryExpr = deduceType expr tm
   in case tryExpr of
        Right e -> Right e
...
foldBlocks curBlocks nextBlock = case curBlocks of
  Right e -> Right e
  Left (eBlocks, tm) ->
    let tryBlock = deduceBlock nextBlock tm
     in case tryBlock of
          Right e -> Right e
